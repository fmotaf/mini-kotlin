...
def gerar_tokens(codigo_fonte):
    ...
    # ESSENTIAL TOKENS:
        ...
        # tokens for punctuation symbols, such as left and right parentheses, comma and semicolon
    ...
    token_specification = [
        ('KEYWORD',  r'^if\b|^else\b|^while\b|^for\b|^fun\b|^return\b'), #palavras reservadas
        ('TYPE',     r'^int\b|^float\b|^void\b'),#tipos de dados
        ('IDENTIFIER', r'^var\b|^val\b'), # Identifiers são nomes dados para variaveis, funcoes, arrays, ou outros tipos definidos pelo usuário. Eles são usados para referenciar esses elementos no código. Ex: var nome = "João" -> 'nome' é um identificador que referencia a variável que armazena o valor "João".
        ('CONSTANT', r'^const\b'),
        ('MATH_OPERATORS', r'^+\b|^-\b|^*\b|^/\b'),
        ('RELATIONAL_OPERATORS', r'^>\b|^<\b|^==\b|^!=\b'),
    ]

    token_specification = [
        ('TYPE',     r'^Byte\b|^Short\b|^Int\b|^Long\b|^Float\b|^Double\b|^Char\b|^String\b|^Boolean\b'),#tipos de dados
        ('KEYWORD',  r'^if\b|^else\b|^while\b|^for\b|^fun\b|^return\b'), #palavras reservadas
        ('STATEMENT', r'^var\b|^val\b'), # Declaração de variáveis
        ('FUNCTION',  r'^fun\b'),        # Declaração de funções
